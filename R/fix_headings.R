...
fix_headings <- function(sub_dir) {
  ...
  for (rmd in rmds) {
    ...
    questions <- stringr::str_extract_all(lines[actual_part_locs],
                                          "[Qq]\\w+ ?\\d") %>% unlist
    new_questions <- paste0("\n", unique(questions), "\n")
    new_question_locs <- sort(actual_part_locs[!duplicated(questions)],
                              decreasing = TRUE)
    for (i in seq_along(new_question_locs)) {
      append(lines, new_questions[i], new_question_locs[i])
    }
    actual_part_locs <- grep("Q\\w+.*\\d\\w", lines)
    parts <- stringr::str_extract_all(lines[actual_part_locs],
                                      "(?<=(\\w ?\\d))\\w") %>% unlist
    ...
    before <- headings - 1
    after <- headings + 1
    need_blank <- sort(c(before[lines[before] != ""], headings[lines[after] != ""]),
                       decreasing = TRUE)
